...
## Marketing site wiring (bigfivegroup.africa)

| Piece | Location |
|-------|----------|
| URL builders | `app/lib/saStorefront.ts` |
| Sales UI | `app/components/FoodsSalesPortal.tsx` → `/foods#shop` |
| Catalog proxy | `GET /api/foods/catalog` → SA `/api/storefront/big-five-foods/products` |
| Quote proxy | `POST /api/foods/quote` → SA `/api/storefront/big-five-foods/quotes` |
| Defaults | Production SA store + API base (override via env) |
...
6. Marketing `/foods#shop` “Order on SA” opens correct store/product deep links  
7. Marketing catalog badge shows **SupplierAdvisor® live** when API responds  
