...
export default function PageHero({
  ...
  return (
    <section className="relative min-h-[min(85dvh,680px)] sm:min-h-[min(90dvh,780px)] md:min-h-[min(92dvh,820px)] flex items-center justify-center overflow-hidden">
      <div
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-14 sm:py-20 md:py-24">
        {logo && (
        ...
        <div className="inline-flex max-w-full items-center gap-2 px-3 sm:px-4 py-1.5 rounded-full border border-white/25 text-[10px] sm:text-[11px] tracking-[2px] sm:tracking-[3px] text-white/90 mb-4 sm:mb-6">
          <span className="truncate">{eyebrow}</span>
        </div>
        <h1 className="text-white text-[clamp(1.75rem,5vw+0.6rem,4.5rem)] font-semibold tracking-tighter leading-[1.05] sm:leading-[0.98] mb-4 sm:mb-6 text-balance px-1">
          {title}
        </h1>
        <p className="max-w-2xl mx-auto text-base sm:text-lg md:text-2xl text-white/90 font-light tracking-tight mb-7 sm:mb-10 text-pretty px-1">
          {subtitle}
        </p>
        {ctas.length > 0 && (
          <div
            className={`flex flex-col gap-3 sm:gap-4 justify-center items-stretch mx-auto w-full ${
              manyCtas
