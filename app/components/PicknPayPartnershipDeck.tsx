...
import {
  PICK_N_PAY_PARTNERSHIP,
  buildPicknPayImpactReport,
  formatZar,
  formatZarCompact,
  formatZarPrecise,
} from "../lib/picknPayPartnership";

const theme = DECK_THEMES.picknpay;
const TOTAL = 20;
const P = PICK_N_PAY_PARTNERSHIP;
const EX = P.giving.example;
const M = P.picknPayMargin;
const RANGES = P.mandelaPackRanges;
const IMPACT = buildPicknPayImpactReport();

/**
 * PRODUCT Pick n Pay CAN TRUST — one pack per category.
...
/**
 * Pick n Pay-forward co-brand.
 * Pick n Pay logo is a wide wordmark; Big Five Foods mark is nearly square — never force
...
function CoBrandRow({ light }: { light?: boolean }) {
  ...
  const pnpSrc = "/partners/pick-n-pay-logo.png";
  ...
  return (
    <div className="flex flex-nowrap items-center gap-2.5 sm:gap-4 mb-3 sm:mb-5 max-w-full">
      ...
      >
        {pdf ? (
          <DeckPrintImage src={pnpSrc} alt="Pick n Pay" paddingClass="p-1" fit="contain" />
        ) : (
          <Image src={pnpSrc} alt="Pick n Pay" fill className="object-contain p-1" sizes="176px" priority />
        )}
      </div>
      <span
...
function Slide({ index }: { index: number }) {
  ...
  switch (index) {
              ...
              <DeckTitleLayout>
                <div>
                  <DeckEyebrow light theme={theme}>
                    Pick n Pay SOUTH AFRICA · PARTNERSHIP PRESENTATION · CONFIDENTIAL
                  </DeckEyebrow>
                  <CoBrandRow light />
                  ...
                  <p className="text-white/75 max-w-2xl mt-3 sm:mt-4 text-xs sm:text-sm md:text-base leading-relaxed">
                    ...
                    Ring-fence 10% of partnership turnover (Pick n Pay 5% + Big Five Foods 5%) for Restore
                ...
                <div className="text-white/45 space-y-0.5 text-[10px] sm:text-xs">
                  <p>Pick n Pay leads the narrative · Big Five Foods supplies the product</p>
                  <p>bigfivegroup.africa · /partner/pick-n-pay#pick-n-pay-partnership-deck</p>
    ...
    case 1:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>A Pick n Pay story of good — commercially sound</DeckTitle>
          <ol className={forPrint ? "space-y-1 max-w-2xl" : "space-y-2 max-w-2xl"}>
            {[
              "Why Pick n Pay should own the food-security narrative",
              "How Pick n Pay does good — feed, fund, stand with foundations",
              "Mandela pack pricing · Pick n Pay’s margin on every sale",
              "Mandela pack products — all four Big Five Foods categories",
              "Sell with purpose · donate with dignity · campaign hybrid",
              "The 10% model · Pick n Pay 5% + Big Five Foods 5%",
              "National impact report — stores, sales, Pick n Pay profit, people fed",
        ...
        <DeckSlideShell dark theme={theme}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            {P.picknPayStory.map((c) => (
              ...
              >
                <div
                  className={`font-semibold text-red-200 mb-1 ${forPrint ? "text-xs" : "text-sm"}`}
                >
                  {c.t}
                </div>
                <p
    ...
    case 4:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>PRICING · Pick n Pay MARGIN</DeckEyebrow>
          <DeckTitle>Two pack tiers — clear Pick n Pay front margin on each</DeckTitle>
          <p
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-0 border-t border-black/5">
                  <div className={`bg-[#0a0a0a] text-white ${forPrint ? "p-2.5" : "p-4"}`}>
                    <div className="text-[9px] font-semibold uppercase text-red-300/90">Buy</div>
                    <div
                  ...
                  <div
                    className={`border-l border-emerald-100 bg-red-50/80 ${
                      forPrint ? "p-2.5" : "p-4"
                    }`}
                  >
                    <div
          ...
          <div
            className={`rounded-xl border border-emerald-100 bg-red-50/60 ${
              forPrint ? "p-2.5 text-[10px]" : "p-3 text-sm"
            } text-[#404040] leading-relaxed mb-2`}
          >
            <strong className="text-black">For Pick n Pay retailers:</strong> {M.detail}
          </div>
          <div
    ...
    case 5:
      return (
        <DeckSlideShell theme={theme}>
          {/* Same product image layout as PRODUCT Pick n Pay CAN TRUST (format, size, grid) */}
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            ...
            <div
              className={`mt-2 shrink-0 rounded-xl border border-red-200 bg-red-50/70 ${
            ...
            >
              ...
              fortified plate story for Pick n Pay shoppers and donation hampers.
            </div>
          </div>
        </DeckSlideShell>
      );
    ...
    case 6:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>HOW Pick n Pay DOES GOOD</DeckEyebrow>
          <DeckTitle>Three ways Pick n Pay turns a pack into community impact</DeckTitle>
          ...
          <p className={`text-[#737373] ${forPrint ? "text-[9px]" : "text-xs"} leading-relaxed`}>
            ...
            logistics. Pick n Pay stays the hero of every story — on the shelf and in the community.
          </p>
        </DeckSlideShell>
      );
    ...
    case 7:
      return (
        <DeckSlideShell dark theme={theme}>
          ...
          <DeckTitle>
            <span className="text-white">
              Pick n Pay commits 5%. Big Five Foods matches 5%. Foundations receive 10%.
            </span>
          </DeckTitle>
          <p
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <DeckStatTile
              ...
              theme={theme}
              value="5%"
              label="Pick n Pay of retail sell-through (at RRP R67)"
            />
            <DeckStatTile
              dark
              theme={theme}
              value="5%"
              label="Big Five Foods matches — of trade to Pick n Pay"
            />
            <DeckStatTile
              dark
              theme={theme}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            >
              <div className={`font-semibold text-red-200 mb-1 ${forPrint ? "text-xs" : "text-sm"}`}>
                Pick n Pay’s 5%
              </div>
              <p className={`text-white/65 leading-snug ${forPrint ? "text-[10px]" : "text-sm"}`}>
                {P.giving.bases.picknPay}
              </p>
            </div>
            <div
              className={`rounded-xl border border-white/10 bg-white/[0.06] ${
    ...
    case 8:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>Pick n Pay ECONOMICS · ILLUSTRATIVE</DeckEyebrow>
          <DeckTitle>Margin for Pick n Pay stores · impact for foundations</DeckTitle>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
            <DeckStatTile
              theme={theme}
              value={M.marginRandLabel}
              label="Pick n Pay front margin per pack @ R67 RRP"
            />
            <DeckStatTile
              theme={theme}
            ...
            <DeckStatTile
              theme={theme}
              value={formatZarPrecise(P.giving.perUnitIllustrative.atRrp.picknPay)}
              label="Pick n Pay 5% contribution per pack sold"
            />
            <DeckStatTile
              theme={theme}
              value={formatZar(EX.picknPayContribution + EX.foodsContribution)}
          ...
          >
            <div
              className={`text-red-300/90 uppercase tracking-wide font-semibold ${
                forPrint ? "text-[9px]" : "text-[10px]"
              }`}
            >
              {EX.label} · Pick n Pay front-margin pool (before overheads & 5% giving)
            </div>
            <div
              className={`font-semibold tracking-tighter tabular-nums ${
            ...
            >
              {formatZar(EX.picknPayMarginPool)}
            </div>
            <p className={`text-white/65 mt-1 ${forPrint ? "text-[10px]" : "text-sm"} leading-relaxed`}>
              Illustrative Pick n Pay front margin on 100,000 packs at RRP (not net profit). Separately,
              Pick n Pay’s 5% contribution ≈ {formatZar(EX.picknPayContribution)} and Big Five Foods’ match ≈{" "}
    ...
    case 9:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>What scale could mean across Pick n Pay South Africa</DeckTitle>
          <p
            ...
            >
              <thead>
                <tr className="border-b border-black/15" style={{ color: theme.accentDark }}>
                  ...
                  <th className="py-1.5 font-semibold text-right">Pick n Pay front margin</th>
                </tr>
              </thead>
              <tbody>
                {IMPACT.tiers.map((t) => (
                  <tr key={t.format} className="border-b border-black/8 text-[#404040]">
                    ...
                    <td className="py-1.5 text-right tabular-nums font-semibold" style={{ color: theme.accentDark }}>
                      {formatZar(t.picknPayFrontMargin)}
                    </td>
                  </tr>
                ))}
                <tr className="bg-red-50/80 font-semibold text-black">
                  <td className="py-1.5 pr-2">National total (~709 Pick n Pay grocery · modelled)</td>
                  <td className="py-1.5 pr-2 text-right tabular-nums">
                  ...
                  <td
                    ...
                    style={{ color: theme.accentDark }}
                  >
                    {formatZarCompact(IMPACT.national.picknPayFrontMargin)}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2 mb-2">
            <DeckStatTile
              theme={theme}
              value={formatZarCompact(IMPACT.national.picknPayFrontMargin)}
              label="Pick n Pay front margin pool / year (before overheads & 5% give)"
            />
            <DeckStatTile
              theme={theme}
              value={formatZarCompact(IMPACT.national.combined10)}
              label="10% ring-fence to foundations (Pick n Pay 5% + Foods 5%)"
            />
            <DeckStatTile
              theme={theme}
          ...
          >
            <div className="rounded-xl border border-black/10 bg-[#fafafa] p-2 sm:p-2.5 text-[#404040] leading-snug">
              <strong className="text-black">Pick n Pay commercial takeaway:</strong> At these velocities,{" "}
              ...
              front margin — with Pick n Pay’s 5% contribution ≈{" "}
              {formatZarCompact(IMPACT.national.picknPayGive5)} still leaving a large commercial pool for
              retailers (front margin is not net profit).
            </div>
            <div className="rounded-xl border border-emerald-100 bg-red-50/50 p-2 sm:p-2.5 text-[#404040] leading-snug">
              ...
              {formatZarCompact(IMPACT.pilot.picknPayFrontMargin)} front margin ·{" "}
    ...
    case 10:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>FOUNDATION · 01 · Pick n Pay STANDS WITH</DeckEyebrow>
          ...
          {"verse" in P.npos[0]! && P.npos[0].verse && (
            <blockquote
              className={`max-w-3xl mb-3 rounded-xl border border-emerald-100 bg-red-50/40 ${
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {P.npos[0]!.focus.map((f) => (
              <span
                key={f}
                className={`rounded-full border border-red-200 bg-red-50 font-semibold ${
          ...
          >
            <strong className="text-black">Pick n Pay’s role:</strong> Sell packs that fund care — and
            donate product into child- and community-facing programmes. SA Harvest can deliver
            Pick n Pay-donated product into RAF feeding pathways so meals arrive with reliability.
          </div>
        </DeckSlideShell>
      );
    ...
    case 11:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>FOUNDATION · 02 · Pick n Pay STANDS WITH</DeckEyebrow>
          <div className="flex flex-wrap items-start gap-4 mb-3">
            <div
              className={`relative bg-white rounded-xl border border-black/10 shrink-0 ${
          ...
          >
            <strong className="text-black">Pick n Pay’s role:</strong> Dignity on the plate for women and
            children in care pathways — product donations and contribution support that Pick n Pay
        ...
        <DeckSlideShell theme={theme}>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {P.npos[2]!.focus.map((f) => (
              <span
                key={f}
                className="rounded-full border border-red-200 bg-red-50 text-xs font-semibold px-2.5 py-1"
                style={{ color: theme.accentDark }}
              >
                {f}
              </span>
            ...
            {[
              {
                ...
                d: "Reliable last-mile so Pick n Pay-donated fortified food reaches local kitchens that feed daily.",
              },
              {
          ...
          <div className="rounded-xl border border-emerald-100 bg-red-50/50 p-3 sm:p-4 text-sm text-[#404040] leading-relaxed">
            <strong className="text-black">How the chain works:</strong> Pick n Pay sells or donates · Big
    ...
    case 13:
      return (
        <DeckSlideShell theme={theme}>
          <DeckEyebrow theme={theme}>WHAT Pick n Pay WINS</DeckEyebrow>
    ...
    case 14:
      return (
        <DeckSlideShell theme={theme}>
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            <div className="shrink-0">
              <DeckEyebrow theme={theme}>PRODUCT Pick n Pay CAN TRUST</DeckEyebrow>
              <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold tracking-tighter text-black text-balance mb-2 sm:mb-3">
                Fortified African food ready for Pick n Pay shelves and foundations
              </h2>
            </div>
    ...
    case 15:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>Protect Pick n Pay’s reputation while the good scales</DeckTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            {[
              {
                icon: Scale,
                t: "Term sheet first",
                d: "SKUs, R45 / R67, Pick n Pay margin discipline, 5%+5% giving, NPO split, brand use for Pick n Pay-led storytelling.",
              },
              {
                icon: Building2,
                t: "Transparent reporting Pick n Pay can share",
              ...
              },
              {
                ...
                d: "Shelf and campaign copy celebrates Pick n Pay doing good without over-claiming meal counts until yields are locked.",
              },
            ].map((it) => (
              <div
                key={it.t}
    ...
    case 16:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>From Pick n Pay pilot to a national rhythm of good</DeckTitle>
    ...
    case 17:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>What we will put in writing with Pick n Pay</DeckTitle>
          <ul className={forPrint ? "space-y-1.5" : "space-y-2"}>
            {P.honesty.map((h) => (
              <li
                key={h}
                className={`flex gap-2 rounded-xl border border-emerald-100 bg-red-50/50 text-[#404040] ${
                  forPrint ? "p-2 text-[10px]" : "p-3 text-sm"
                }`}
              >
                <span className="text-red-900 shrink-0 font-semibold">·</span>
    ...
    case 18:
      return (
        <DeckSlideShell theme={theme}>
          ...
          <DeckTitle>Help Pick n Pay lead — five decisions to start</DeckTitle>
          <ol className={forPrint ? "space-y-1.5" : "space-y-2"}>
            {[
              "Pilot listing of the Nelson Mandela pack (and agreed SKUs) in selected Pick n Pay clusters",
              "Approve trade R45 ex. VAT and RRP R67 incl. VAT — with Pick n Pay front-margin discipline",
              "Adopt Pick n Pay 5% + Big Five Foods 5% giving to Restore Africa Foundation and A Heart To Help",
              "Authorise donation PO pathway with SA Harvest last-mile delivery to soup kitchens, feeding schemes, RAF and AHTH",
              "Nominate Pick n Pay commercial + CSI leads for a 30-day term-sheet and campaign sprint",
            ].map((a, i) => (
          ...
          <div className="relative h-full w-full min-h-0">
            ...
            <div className="relative z-10 h-full">
              <DeckTitleLayout>
                <div>
                  <DeckEyebrow light theme={theme}>
                    NEXT STEP · Pick n Pay DOES GOOD
                  </DeckEyebrow>
                  <CoBrandRow light />
                  <h2
                  ...
                  >
                    ...
                    Big Five Foods as product partner behind Pick n Pay’s story of good.
                  </p>
                ...
                <div className="space-y-3">
                  <a
                    href={`mailto:${P.contactEmail}?subject=${encodeURIComponent(
                      "Pick n Pay × Big Five Foods — Pick n Pay does good partnership"
                    )}&body=${encodeURIComponent(
                      "Hello Big Five team,\n\nI would like to progress the Pick n Pay partnership (Mandela pack, Pick n Pay margin, 10% foundations model).\n\nName:\nRole / region:\nPreferred pilot stores:\n\nThank you."
                    )}`}
                  ...
                  <div className={`text-white/45 ${forPrint ? "text-[9px]" : "text-xs"}`}>
                    bigfivegroup.africa/foods · /partner/pick-n-pay#pick-n-pay-partnership-deck · Confidential
                  </div>
                </div>
              </DeckTitleLayout>
            </div>
...
export default function PicknPayPartnershipDeck() {
  return (
    <div id="pick-n-pay-partnership-deck" className="scroll-mt-28 w-full min-w-0">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-6 sm:mb-8 text-center">
        <div className="flex justify-center mb-4">
          <div className="relative h-12 sm:h-14 w-40 sm:w-52 bg-white rounded-xl border-2 border-[#006633]/20 px-3 py-1 shadow-sm">
            <DeckPrintImage src="/partners/pick-n-pay-logo.png" alt="Pick n Pay" paddingClass="p-1" fit="contain" />
          </div>
        </div>
        <div
          ...
          style={{ color: "#006633" }}
        >
          PICK N PAY PARTNERSHIP PRESENTATION · {TOTAL} SLIDES · PRIVATE · Pick n Pay ONLY
        </div>
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tighter text-black mb-2 text-balance">
          Pick n Pay does good — partnership deck
        </h2>
        <p className="text-sm sm:text-base text-[#525252] max-w-2xl mx-auto leading-relaxed">
          Private briefing for Pick n Pay: store margin, Mandela pack pricing, sell-or-donate pathways,
          10% to foundations — with Pick n Pay as the face of community good.
        </p>
      </div>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <DeckShell
          id="pick-n-pay-partnership-deck-shell"
          printRootId="pick-n-pay-partnership-deck-print-root"
          total={TOTAL}
          theme={theme}
          eyebrow="PICK N PAY PARTNERSHIP DECK"
          title="Pick n Pay does good — Partnership Presentation"
          description="Pick n Pay-led narrative: store margin, Mandela pack, 10% to NPOs, sell or donate."
          sharePath="/partner/pick-n-pay#pick-n-pay-partnership-deck"
          shareTitle="Pick n Pay does good — partnership"
          shareText="Pick n Pay partnership briefing: fortified food on shelf, healthy store margin, 10% to foundations."
          renderSlide={(i) => <Slide index={i} />}
        />
      </div>
      <p className="mt-4 text-center text-xs text-[#737373] px-4 max-w-2xl mx-auto">
        Keyboard: ← → · Private to Pick n Pay partner logins ·{" "}
        <span className="font-medium text-black">/partner/pick-n-pay#pick-n-pay-partnership-deck</span>
        {" · "}
