...
function InvestorLoginForm() {
  ...
  const from = searchParams.get("from") || "/investor";
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.replace(from.startsWith("/investor") ? from : "/investor");
      router.refresh();
    } catch {
  ...
  return (
    <div className="w-full max-w-md mx-auto min-w-0">
      ...
      <div className="text-[10px] sm:text-xs tracking-[2px] sm:tracking-[3px] text-amber-400/90 mb-3">
      ...
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tighter text-white mb-3 text-balance">
        Sign in with your investor email
      </h1>
      <p className="text-sm sm:text-base text-white/65 leading-relaxed mb-8 text-pretty">
        This area is restricted to authorised investors. Enter the email address Big Five has
        registered for you.
      </p>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <label className="block">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/15 bg-white/5 pl-10 pr-4 py-3.5 text-sm sm:text-base text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400/40 min-w-0"
            />
          </div>
        </label>
        ...
        >
          {loading ? "Checking access…" : "Enter investor portal"}
      ...
      <p className="mt-8 text-xs text-white/40 leading-relaxed">
        Need access? Email{" "}
        <a
          href="mailto:craig@bigfivegroup.africa?subject=Investor%20portal%20access"
...
export default function InvestorLoginPage() {
  return (
    <div className="page-shell min-h-[calc(100dvh-var(--navbar-height))] bg-[#0a0a0a] text-white flex items-center">
